...
from audita import (amortizaciones, analiticos, bitacora, comparador, cuadres,  # noqa: E402
                  estado, excel_out, financiacion, leasing, materialidad,
                  muestreo, perfil, plan_contable)
from audita.encargo import Encargo  # noqa: E402
from audita.excepciones import BLOQUEANTE, RESOLVER  # noqa: E402
...
def t_presentacion() -> None:
    ...
    from audita.excepciones import Excepcion, Resultado
    from audita.traza import RegistroTrazas, Traza
...
def t_cli() -> None:
    ...
    afirma(code == 0, "`audita doctor` termina sin bloqueantes en esta instalacion")
    ...
    afirma(code == 0 and os.path.exists(os.path.join(enc, "encargo.json")),
           "`audita nuevo` crea el encargo y su estructura de carpetas")
    ...
    afirma(code == 0 and "Todos los cuadres" in salida,
           "`audita ingesta` pasa los cuadres y genera el papel 2.1")
    ...
    afirma(code == 0 and "Reserva de capitalizacion" in salida,
           "`audita reservas` identifica la reserva de capitalizacion como indisponible")
    ...
    afirma(code == 0 and "Materialidad de ejecucion" in salida,
           "`audita materialidad` calcula y registra la materialidad")

    code, salida = corre("estado", enc)
    afirma(code == 0 and "SIGUIENTE PASO RECOMENDADO" in salida,
           "`audita estado` presenta el panel con el siguiente paso")

    code, salida = corre("pbc", enc, "--anadir", "Cuadros de leasing", "--area", "F",
                         "--prioridad", "1")
    afirma(code == 0 and "P001" in salida, "`audita pbc` registra un pendiente del cliente")

    code, salida = corre("horas", enc)
    afirma(code == 0 and "2.5" in salida, "`audita horas` refleja las horas imputadas")

    code, salida = corre("validar", enc, "--listar")
    afirma(code == 0 and "PENDIENTE-VALIDACION" in salida,
           "`audita validar --listar` muestra las ejecuciones sin validar")
    code, _ = corre("validar", enc, "--entrada", "IA-0001", "--quien", "MJ Perez")
    afirma(code == 0, "`audita validar` valida una entrada concreta")

    code, salida = corre("calidad", enc, "--pre-vuelo")
    afirma("PANEL DEL SOCIO" in salida, "`audita calidad` emite el panel del socio")
    ...
    afirma(code == 0 and "Test de asientos" in salida, "`audita asientos` se ejecuta")

    code, salida = corre("leasing", os.path.join(FIX, "leasings_internos.xlsx"),
                         "2025-12-31")
    afirma("contratos" in salida, "`audita leasing` procesa el lote")

    code, salida = corre("amortizaciones", os.path.join(FIX, "inventario_inmovilizado.xlsx"),
                         "2025-01-01", "2025-12-31")
    afirma("Recalculados" in salida, "`audita amortizaciones` recalcula el inventario")
    ...
    afirma("instrumentos" in salida, "`audita financiacion` recalcula la cartera")
    ...
    afirma("4,850.00" in salida.replace(".", ".") or "4850" in salida.replace(",", ""),
           "`audita comparar` detecta la diferencia deliberada")

    code, salida = corre("muestreo", sys_y_saldos, "saldo", "--metodo", "dirigido",
                         "--materialidad", "15031")
    afirma("Semilla de aleatoriedad" in salida,
           "`audita muestreo` publica la semilla para que sea reproducible")
    ...
    afirma(code == 0 and "PERFIL:" in salida and "Honorarios estimados" in salida,
           "`audita estimar` calcula perfil, horas y honorarios")
    ...
    afirma(code == 0 and "umbral de investigacion" in salida.lower(),
           "`audita analiticos` publica el umbral fijado a priori")
...
def t_menu() -> None:
    ...
    sin_hint = [os.path.basename(os.path.dirname(f)) for f, fm in skills
                ...
                and os.path.basename(os.path.dirname(f)) != "convenciones-despacho"]
    ...
    procs = {os.path.splitext(os.path.basename(p))[0]
             for p in g.glob(os.path.join(raiz, "procedimientos", "*.md"))}
    afirma(len(procs) >= 25, f"los procedimientos siguen en procedimientos/ (hay {len(procs)})")
    rotos = []
    for f, fm in skills:
...
def t_paquete_claude_ai() -> None:
    ...
    r = subprocess.run([sys.executable, os.path.join(RAIZ, "scripts", "empaquetar_skill.py")],
    ...
    zpath = os.path.join(RAIZ, "dist", "auditoria-nia-es.skill")
    zzip = os.path.join(RAIZ, "dist", "auditoria-nia-es.zip")
    ...
    with zipfile.ZipFile(zpath) as z:
        ...
        afirma(skill == ["auditoria-nia-es/SKILL.md"],
        ...
        # ${CLAUDE_PLUGIN_ROOT} ni el lanzador `audita` en el PATH
    ...
    zcc = os.path.join(RAIZ, "dist", "auditoria-nia-es-claude-code.zip")
    afirma(os.path.exists(zcc), "se genera el .zip para Claude Code")
    with zipfile.ZipFile(zcc) as z:
        nombres = set(z.namelist())
        for req in ("auditoria-nia-es/.claude-plugin/plugin.json",
                    "auditoria-nia-es/bin/audita",
                    "auditoria-nia-es/scripts/audita/cli.py",
                    "auditoria-nia-es/procedimientos/materialidad.md",
                    "auditoria-nia-es/referencias/mapeo-pgc.json",
                    "auditoria-nia-es/plantillas/informe-auditoria.md"):
            afirma(req in nombres, f"el paquete de Claude Code incluye {req}")
        afirma(sum(1 for n in nombres
                   if n.startswith("auditoria-nia-es/skills/") and n.endswith("SKILL.md")) == 10,
               "viajan las 10 skills")
        afirma((z.getinfo("auditoria-nia-es/bin/audita").external_attr >> 16) & 0o111,
               "el lanzador bin/audita conserva el bit de ejecucion")
        afirma(not any("/dist/" in n or "__pycache__" in n or n.endswith(".pyc")
                       for n in nombres),
               "no viajan artefactos de construccion ni cache de Python")

def t_privacidad() -> None:
    """Nada del despacho ni de sus clientes puede acabar en el repositorio.

    El deber de secreto del art. 31 LAC no admite un descuido: un NIF real o un
    papel de trabajo en un repositorio publico no se arregla con un commit de
    correccion.
    """
    bloque("privacidad del repositorio")
    import subprocess
    r = subprocess.run([sys.executable,
                        os.path.join(RAIZ, "scripts", "comprobar_privacidad.py")],
                       capture_output=True, text=True, cwd=RAIZ)
    afirma(r.returncode == 0,
           f"no hay datos privados versionados ({r.stdout.strip()[-90:]})")

    # las funciones de validacion tienen que reconocer un identificador real:
    # si no validan, el verificador no encontraria nunca nada
    sys.path.insert(0, os.path.join(RAIZ, "scripts"))
    import importlib.util
    spec = importlib.util.spec_from_file_location(
        "cp", os.path.join(RAIZ, "scripts", "comprobar_privacidad.py"))
    cp = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(cp)

    # NIF: 8 digitos + letra de la tabla TRWAGMYFPDXBNJZSQVHLCKE
    n = 12345678
    afirma(cp.nif_valido(f"{n}{'TRWAGMYFPDXBNJZSQVHLCKE'[n % 23]}"),
           "reconoce un NIF con letra de control correcta")
    afirma(not cp.nif_valido("12345678A"), "rechaza un NIF con letra incorrecta")
    afirma(cp.nie_valido(f"X{1234567:07d}{'TRWAGMYFPDXBNJZSQVHLCKE'[int('0' + '1234567') % 23]}"),
           "reconoce un NIE con letra de control correcta")
    afirma(cp.cif_valido(cif_sintetico_local()), "reconoce un CIF con digito de control correcto")
    afirma(not cp.cif_valido("B12345678"), "rechaza un CIF con digito incorrecto")
    afirma(not cp.iban_valido("ES0000000000000000000000"), "rechaza un IBAN mal formado")

    # y los ficheros que no deben versionarse jamas
    for prohibido in ("encargos/ACME/2025/encargo.json", "control.xlsx",
                      "clientes/x.pdf.csv", "certificado.pfx",
                      "config/configuracion.md", "referencias/tarifas.json"):
        afirma(cp.PROHIBIDOS.search(prohibido) is not None,
               f"bloquea {prohibido}")
    afirma(cp.PROHIBIDOS.search("referencias/tarifas-ejemplo.json") is None,
           "no bloquea el fichero de ejemplo de tarifas")


def cif_sintetico_local() -> str:
    cuerpo = "0000000"
