...
1. Descargue `dist/auditoria-nia-es.skill` del repositorio (o el `.zip`, que
...
Lo normal es el marketplace, ahora que el plugin está en la raíz del
repositorio:
...
Si eso falla, descarga `dist/auditoria-nia-es-claude-code.zip` y descomprímelo
en tu directorio de skills. **No hace falta marketplace ni git**, así que no hay
sincronización que pueda fallar:
...
```bash
claude plugin list      # auditoria-nia-es@skills-dir · 1.6.0 · loaded
```
...
Debe decir «El plugin es operativo». El lanzador `audita` se añade solo al `PATH`
mientras el plugin está activo: no tienes que configurar rutas.

Para probarlo sin instalarlo: `claude --plugin-dir /ruta/a/Auditoria/auditoria-nia-es`.

## 2. Configurar (20 minutos, una sola vez)

Abre `skills/convenciones-despacho/SKILL.md` —es el fichero de configuración del
...
- **Tarifas por categoría** → copia `referencias/tarifas-ejemplo.json` a
...
`[VERIFICAR-LITERAL-ICAC]` en `plantillas/informe-auditoria.md`. Luego borra
...
```bash
audita estado <encargo>                    # ¿dónde estamos y qué toca ahora?
audita pbc <encargo>                       # ¿qué falta del cliente?
audita horas <encargo>                     # ¿cómo va el presupuesto?
...
| Los honorarios salen `[PENDIENTE-CLIENTE]` | Falta `referencias/tarifas.json` |
| Diferencias sistemáticas de tipo en leasings | Comisiones de apertura no incluidas en el cálculo del banco. Ver `ARR-020` |

| `audita: command not found` | El plugin no está activo. Compruébalo con `/plugin list`; si no aparece, reinstálalo |
| `ERROR: faltan dependencias` | `pip install pandas openpyxl`. El lanzador dice cuál falta |
| El informe de calidad avisa de ejecuciones sin validar | Es lo correcto: valide cada una con `audita validar <encargo> --entrada IA-000X --quien "..."` |

Para ver la traza completa de un error: `AUDITA_DEBUG=1 python3 -m audita.cli ...`

## 8. Una rutina que merece la pena

Al cerrar cada área: `--horas` en el comando, validar la entrada de la bitácora,
y `audita estado`. Son treinta segundos y evitan que el último día aparezcan a la
