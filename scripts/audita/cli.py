...
calculo sea reproducible por un revisor ejecutando el mismo comando.
    ...
    python -m audita.cli <subcomando> --help
"""
...
def _bitacora(args, skill: str, entradas: list[str], salidas: list[str],
    ...
    print(f"Bitacora: {eid} registrada en uso-ia.log "
          f"(pendiente de validacion: `audita validar {carpeta} --entrada {eid} --quien \"...\"`)")
    return eid
...
def cmd_doctor(args) -> int:
    ...
    """
    # .../<plugin>/scripts/audita/cli.py -> cuatro niveles hasta la raiz.
    # El lanzador `audita` la exporta en AUDITA_RAIZ; al invocar `python -m audita.cli`
    # directamente hay que deducirla, y equivocarse aqui hace que doctor declare
    # ausentes ficheros que si estan.
    raiz = os.environ.get("AUDITA_RAIZ") or os.path.abspath(
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..", ".."))
    bloqueantes: list[str] = []
    avisos: list[str] = []
    print("auditoria-nia-es - comprobacion de la instalacion")
    print("=" * 70)
    ...
    conf = next((c for c in (os.path.join(raiz, "skills", "convenciones-despacho", "SKILL.md"),
                             os.path.join(raiz, "procedimientos", "convenciones-despacho.md"))
                 if os.path.exists(c)), "")
    if not conf:
        ...
        if pendientes:
            avisos.append(
                f"{pendientes} campos sin completar en skills/convenciones-despacho/SKILL.md "
    ...
    if not (dir_ref and os.path.exists(os.path.join(dir_ref, "tarifas.json"))):
        avisos.append("No hay tarifas.json: `estimacion-encargo` calculara horas pero "
                      ...
                      "referencias/tarifas-ejemplo.json y ponga las suyas.")
...
def construye_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="audita", description="Libreria de calculo determinista de auditoria-nia-es.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if os.environ.get("AUDITA_DEBUG"):
            raise
        return 1
