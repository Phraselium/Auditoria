...
Equipo de auditoría senior virtual para **despachos españoles**. Cubre el ciclo
...
**Estado:** v1.6.0 · 10 skills · 29 procedimientos · 1 comando · 3 agentes ·
...
1. Descargue **`build/auditoria-nia-es-claude-code.zip`** (299 KB) del repositorio.
2. Descomprímalo dentro de `~/.claude/skills/`:

```bash
mkdir -p ~/.claude/skills
unzip -o ~/Descargas/auditoria-nia-es-claude-code.zip -d ~/.claude/skills/
```

En Windows: descomprima en `C:\Users\<usuario>\.claude\skills\`, de modo que
quede `…\.claude\skills\auditoria-nia-es\.claude-plugin\plugin.json`.

3. Reinicie Claude Code y compruébelo:

```bash
claude plugin list      # debe decir auditoria-nia-es@skills-dir · 1.6.0 · loaded
```
...
> (`/plugin uninstall auditoria-nia-es@audita`): la copia instalada tiene precedencia y
> la del directorio de skills no llegaría a cargarse.

### Opción B — abrir el repositorio en Claude Code

El repositorio se declara a sí mismo. `.claude/settings.json` lleva el
marketplace y el plugin, así que basta con abrir la carpeta:
...
Al confiar en la carpeta, Claude Code ofrece instalarlo. Es la vía natural para
que un compañero del despacho lo tenga sin explicarle nada: clona y abre.

### Opción C — desde el marketplace

```
/plugin marketplace add Phraselium/Auditoria
/plugin install auditoria-nia-es@audita
```

Si el resumen de instalación dice `Run /reload-plugins to activate`, ejecútalo.

**Si falla la sincronización**, en este orden:

1. **Borre la caché de plugins.** Es el remedio documentado y el que más veces
   resuelve un fallo de sincronización:

   ```bash
   rm -rf ~/.claude/plugins/cache
   ```

2. **Fije la rama explícitamente** con `#`, en lugar de dejar que resuelva la
   rama por defecto del repositorio:

   ```
   /plugin marketplace remove audita
   /plugin marketplace add https://github.com/Phraselium/Auditoria.git#main
   /plugin install auditoria-nia-es@audita
   ```

3. **Compruebe si el problema es de git**, no del marketplace:

   ```bash
   git clone https://github.com/Phraselium/Auditoria.git /tmp/prueba-audita
   ```

   Si este clonado falla, es red o credenciales, y **todas** las vías con
   marketplace fallarán. Use la opción A.

### Opción D — desde una copia local ya clonada
...
### Opción E — desde la URL del zip, sin clonar

```bash
claude --plugin-url https://raw.githubusercontent.com/Phraselium/Auditoria/main/auditoria-nia-es/build/auditoria-nia-es-claude-code.zip
```

Claude Code descarga el archivo al arrancar y lo carga **solo para esa sesión**.
Útil para probarlo en una máquina ajena sin dejar nada instalado.

### Opción F — probarlo sin instalar

```bash
claude --plugin-dir /ruta/a/Auditoria/auditoria-nia-es
```

La copia local tiene prioridad sobre la instalada, así que sirve para probar
cambios sin desinstalar nada.

### Para actualizar a una versión nueva (opciones C y D)

**Sincronizar el marketplace no actualiza el plugin ya instalado.** Hay que
reinstalarlo, y en este orden:

```
/plugin marketplace update audita
/plugin uninstall auditoria-nia-es@audita
/plugin install auditoria-nia-es@audita
```
...
### Después de instalar, en cualquiera de los casos
...
```bash
audita doctor                            # comprueba instalación y configuración
```

`audita doctor` distingue lo que **impide** trabajar (falta Python o una
...
Fuera de campaña puede desactivarlo con `/plugin disable auditoria-nia-es` y
reactivarlo con `/plugin enable auditoria-nia-es`.

Verificación completa de la librería de cálculo:

```bash
claude plugin validate <ruta>/auditoria-nia-es    # debe decir "Validation passed"
cd <ruta>/auditoria-nia-es && python3 tests/run_all.py   # 273/273 y cobertura 100 %
```

Después, **completa `skills/convenciones-despacho/SKILL.md`**: los campos entre `«»` son los datos reales del
...
`shared/procedimientos/`. Las otras cinco —`convenciones-despacho`,
...
```
/auditoria-nia-es:nuevo-encargo "ACME SL" 2025 PGC-PYMES
        │
...
├── skills/convenciones-despacho/SKILL.md                    # perfil del despacho, convenciones y umbrales
├── GUIA-ARRANQUE.md             # una página para empezar
├── bin/audita                     # lanzador; se añade al PATH del Bash
...
└── shared/
    ...
    ├── scripts/audita/            # 23 módulos: ingesta, cuadres, muestreo, leasing…
...
Para la cartera de un despacho de PYME será, en la práctica totalidad de los casos, la redacción
...
El plugin expone el lanzador **`audita`** en el `PATH` del Bash mientras está
activo — no hay que exportar `PYTHONPATH` ni saber dónde se instaló:

```bash
audita doctor          # empiece siempre por aquí
audita --help
```
...
- **El estimador no está calibrado** con datos reales del despacho. Las horas base son
