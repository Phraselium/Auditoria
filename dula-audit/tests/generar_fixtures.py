...
def genera_leasings() -> list[str]:
    ...
    rutas.append(_escribe(
        ...
        preambulo=[["BANCO DEL NORTE, S.A."],
                   ...
                   ["Cliente: TRANSPORTES SINTETICOS SL   NIF: B00000000"],
...
def genera_memoria_y_ccaa(sys_df: pd.DataFrame, resultado: float) -> dict:
    ...
    aceptacion nº3.
    """
    from dula import comparador  # import diferido: el path lo fija el test
...
def genera_encargo_con_defectos(resultado: float) -> str:
    ...
    """
    from dula.encargo import Encargo
...
if __name__ == "__main__":
    import sys
    sys.path.insert(0, os.path.join(os.path.dirname(AQUI), "shared", "scripts"))
