...
audita comparar --ccaa 00-fuentes/ccaa.json \
...
audita comparar --ccaa ccaa.json --anterior-ccaa ccaa_anterior.json \
    --depositadas ccaa_depositadas.json

# 4. Informe de gestion <-> cuentas anuales (NIA-ES 720R)
audita comparar --ccaa ccaa.json --informe-gestion informe_gestion.json

# 5. Documentacion soporte <-> registro contable
audita comparar --soporte facturas.xlsx --contabilidad mayor_400.xlsx \
    --clave "n factura" --columna-importe importe

# 6. Diff entre borradores
audita comparar --borrador-anterior v1.json --borrador-nuevo v2.json

# 7. ULTIMA VERIFICACION antes de la firma
audita comparar --informe informe.json --ccaa-definitivas ccaa_def.json \
...
`referencias/desgloses-memoria.json`, que recoge las 25 notas por modelo
