...
# Convenciones, umbrales y configuración del despacho
...
| | |
|---|---|
| Denominación | «AUDITA AUDITORES, S.L.» |
...
En la práctica totalidad de la cartera de un despacho de PYME será la **redacción de entidad NO
...
Cuatro hojas, siempre, generadas por `scripts/audita/excel_out.py`:
...
### 4.1 Materialidad — `scripts/audita/materialidad.py`
...
### 4.3 Muestreo — `scripts/audita/muestreo.py`
...
### 4.4 Analíticos — `scripts/audita/analiticos.py`
...
### 4.5 Festivos para el test de asientos — `scripts/audita/asientos.py`
...
Guárdelo como `referencias/tarifas.json`.
...
registre en `referencias/historico-encargos.json` los **drivers y las horas
...
Verificación de la instalación: `audita doctor` debe decir «El plugin es
